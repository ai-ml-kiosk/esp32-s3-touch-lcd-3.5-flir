...
External TFT, external touch, and separate microSD wiring are not part of this
...
|---|---|---|---|
| Onboard LCD/touch path | User interface | Built-in 320x480 LCD/touch and backlight, used as 480x320 landscape by default | Already wired by Waveshare; use the board support pins and drivers. |
...
- The UI should default to landscape orientation on the built-in 480x320 display.
  ...
  using a fill mode.
- The final display target should preserve the panel's 262K-color capability by
  using an 18-bit RGB666 palette/output path when supported by the Waveshare
  display driver. Use RGB565 only as an explicit fallback if the selected driver
  path cannot push RGB666.
